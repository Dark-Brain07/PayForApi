...
export const claimSchema = z.object({
  ...
  creatorAddress: z.string().regex(/^0x[0-9a-fA-F]{40}$/, "Invalid EVM address").describe("Creator EVM address receiving token claim"),
  amount: z.string().trim().regex(/^\d+(\.\d+)?$/, "Must be numeric string"),
