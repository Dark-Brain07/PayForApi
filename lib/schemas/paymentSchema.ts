...
export const paymentSchema = z.object({
  id: z.string().trim().uuid("Invalid UUID format").min(36).max(50).optional().describe("Unique transaction ID"),
  txHash: z.string().length(66).regex(/^0x[0-9a-fA-F]{64}$/, "Invalid tx hash").describe("Transaction hash"),
  ...
  endpoint: z.string().trim().min(1).max(1000).url("Must be a valid endpoint URL").refine(url => url.startsWith("https://"), "Endpoint must use HTTPS").describe("Target API endpoint URL"),
