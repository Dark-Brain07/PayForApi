    ...
    return (
      <div className="space-y-4">
        ...
        {(articles || []).map((art: Article, i: number) => (
          <div key={art.url || `art-${i}-${art.title ? art.title.slice(0, 10) : 'item'}`} className="p-5 bg-[#0B0E14] rounded-xl border border-[#1E293B] hover:border-[#F5C518]/50 transition-all flex flex-col group shadow-lg">
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {data && typeof data === 'object' && !Array.isArray(data) ? (
          Object.entries(data).map(([coin, info]: [string, { usd?: number }]) => (
            <div key={coin} className="p-6 bg-gradient-to-b from-[#0F172A] to-[#020617] rounded-xl border border-[#00E676]/30 flex flex-col items-center justify-center transform hover:-translate-y-1 transition-transform shadow-lg relative overflow-hidden">
              ...
              <div className="text-3xl font-black text-[#00E676] z-10 tracking-tight">${(info as { usd?: number })?.usd?.toLocaleString(undefined, {minimumFractionDigits: 4, maximumFractionDigits: 4})}</div>
            </div>
          ))
        ) : (
          <p className="text-red-500 col-span-3">Invalid data format for crypto prices.</p>
        )}
      </div>
    );
  }
