...
To qualify, we must avoid "trivial integrations" (like simply adding G$ as a payment token alongside USDm). Instead, we must implement meaningful utility that drives G$ adoption and ecosystem growth.
...
* **The Problem:** The PayForAPI treasury currently takes a 10% cut in USDm for platform sustainability, which doesn't benefit the broader Web3 ecosystem.
