...
export function useAuth(): AuthState & {
  ...
  /**
   * Prompts the user to connect their EVM wallet.
   * @returns {Promise<string | null>} The connected wallet address or null if failed
   */
  const connect = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      setState((s) => ({
        ...s,
        error: isProviderError(err) && err.code === USER_REJECTED_CODE ? "User rejected connection" : ((err as Error)?.message ?? String(err)),
        isConnecting: false,
      }));
      return null;
    }
